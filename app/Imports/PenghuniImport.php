    ...
    SkipsOnFailure
{
    ...
    private function processRow($row, $rowIndex)
    {
        ...
        if ($noSip && (strpos($noSip, '................') !== false || strpos($noSip, 'â€¦.') !== false)) {
            $noSip = null;
        }

        $noSps = isset($row[8]) && !empty($row[8]) ? trim((string)$row[8]) : null;
        if ($noSps && (strpos($noSps, '..................') !== false || strpos($noSps, 'â€¦.') !== false)) {
            $noSps = null;
        }
        ...
        $nilaiJaminan = $this->calculateNilaiJaminan($blok, $lantaiInt, $keringanan);
    ...
    private function calculateNilaiJaminan($blok, $lantaiInt, $keringanan)
    {
        $tarifKeringanan = $this->getTarifKeringanan();
        
        if (in_array($blok, ['A', 'B', 'C'])) {
            if ($lantaiInt && isset($tarifKeringanan[$blok][$lantaiInt]['tidak'])) {
                $tarifTidak = $tarifKeringanan[$blok][$lantaiInt]['tidak'];
                return $tarifTidak * 3;
            }
        } elseif ($blok === 'D') {
            if ($lantaiInt && isset($tarifKeringanan[$blok][$lantaiInt]['normal'])) {
                $tarifNormal = $tarifKeringanan[$blok][$lantaiInt]['normal'];
                return $tarifNormal * 3;
            }
        }
